...
const Page = () => {
  ...
  function postVlog(vlog) {
    ...
    EDIT_VLOGS(vlogid,vlog.title,vlog.description)
      .then((res) => {
        console.log(res);
  ...
  useEffect(()=>{
    const tempTitle = searchparams.get('title').replace('_', ' ');
    const tempDescription = searchparams.get('description').replace('_', ' ');
